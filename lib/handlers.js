...
handlers.index = (data, callback) => {
  //reject any request that isn'ta Get
  if (data.method == 'get') {
...
handlers.favicon = (data, callback) => {
  //reject any request that isn't a data
  if (data.method == 'get') {
        ...
        _data.read('users', phone, (err, userData) => {
          if (!err && userData) {
            _data.delete('users', phone, (err) => {
              if (!err) {
                //Delete all the checks asociated to the user
                var userChecks = typeof userData.checks == 'object' && userData.checks instanceof Array ? userData.checks : []
                var checksDelete = userChecks.length
                if (checksDelete > 0) {
                  var checksDeleted = 0
                  ...
                  userChecks.forEach((checkId) => {
                    ...
                    _data.delete('checks', checkId, (err) => {
                      ...
                      checksDeleted++
                      if (checksDeleted == checksDelete) {
                        if (!deletionsErrors) {
                          callback(200)
                        } else {
                          callback(500, {
                            Error: 'Errors encounters while attenting delete all of the user`s checks, all checks may not have benn deleted from de system successfully'
                          })
                        }
                      }
                    })
...
//Check POSTS
// Required data: protocol, url, method, successCodes, timeoutsSeconds
// Optional Data: none
        ...
        _data.read('users', userPhone, (err, userData) => {
          if (!err && userData) {
            ...
            if (userChecks.length < config.maxChecks) {
              ...
              callback(400, {
                Error: `The user already has the maximun numbers of checks (${ config.maxChecks})`
              })
            }
          } else {
            callback(403)
...
// Checks PUT
// Required data: ID
// Optional Data: protocol, url, method, successCodes, timeoutsSeconds
handlers._checks.put = (data, callback) => {
  ...
  var successCodes = typeof(data.payload.successCodes) == 'object' && data.payload.successCodes instanceof Array && data.payload.successCodes.length > 0 ? data.payload.successCodes : false
  var timeoutSeconds = typeof(data.payload.timeoutSeconds) == 'number' && data.payload.timeoutSeconds % 1 === 0 && data.payload.timeoutSeconds >= 1 && data.payload.timeoutSeconds <= 5 ? data.payload.timeoutSeconds : false
  //Cehck to make sure id is valid
  if (id) {
